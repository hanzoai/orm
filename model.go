...
// models holds metadata for every registered model type.
var models = &registry{
...
// Meta holds parsed metadata for a registered model type.
type Meta struct {
	Kind        string
...
func Lookup(kind string) (*Meta, bool) {
	models.mu.RLock()
	defer models.mu.RUnlock()
	meta, ok := models.byKind[kind]
	return meta, ok
...
func Kinds() []string {
	models.mu.RLock()
	defer models.mu.RUnlock()
	kinds := make([]string, 0, len(models.byKind))
	for k := range models.byKind {
...
func resetRegistry() {
	models.mu.Lock()
	defer models.mu.Unlock()
	models.byKind = make(map[string]*Meta)
	models.byType = make(map[reflect.Type]*Meta)
}
